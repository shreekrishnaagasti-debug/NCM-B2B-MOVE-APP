...
//  Announcements and branch contact details are stored in their own sheets.
...
const CONFIG = {
  ...
  DRIVER_BRANCHES: ["TINKUNE","SATDOBATO","KALANKI","SWOYAMBHU","NAYA BUSPARK","BASUNDHARA","CHABAHIL"]
};
...
const DRIVER_BRANCHES = CONFIG.DRIVER_BRANCHES.slice();

// Driver routing is deliberately separate from shipment routing. Shipment
// sub-branches remain available to branch staff, but drivers only see these
// seven locations and can never check in or out to the old sub-branches.
const DRIVER_DESTINATIONS = {};
DRIVER_BRANCHES.forEach(function(branch) {
  DRIVER_DESTINATIONS[branch] = DRIVER_BRANCHES.filter(function(destination) {
...
/* ═══════════════════════════════════════════════════════
   ANNOUNCEMENTS + BRANCH CONTACTS
   These are intentionally separate from SHIPMENT GPS and
   VAN MOVEMENTS so the existing tracking data is untouched.
 ═══════════════════════════════════════════════════════ */

function getInfoSheet(name, headers) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
...
function infoRows(sheet) {
  var lastRow = sheet.getLastRow();
  return lastRow < 2 ? [] : sheet.getRange(2, 1, lastRow - 1, sheet.getLastColumn()).getValues();
}

function isoInfoDate(value) {
  if (!value) return null;
  var date = value instanceof Date ? value : new Date(value);
  return isNaN(date.getTime()) ? null : date.toISOString();
}

function isDriverBranch(branch) {
  return DRIVER_BRANCHES.indexOf(String(branch || "").toUpperCase().trim()) !== -1;
}

function getAnnouncements() {
  var sheet = getInfoSheet("ANNOUNCEMENTS",
    ["ID","Branch","Message","Created By","Created At","Expires At"]);
  var rows = infoRows(sheet), now = new Date(), result = [];
  for (var i = 0; i < rows.length; i++) {
    var expires = rows[i][5] instanceof Date ? rows[i][5] : new Date(rows[i][5]);
    if (isNaN(expires.getTime()) || expires <= now) continue;
    result.push({
      id: String(rows[i][0] || ""),
      branch: String(rows[i][1] || ""),
      message: String(rows[i][2] || ""),
      createdBy: String(rows[i][3] || ""),
      createdAt: isoInfoDate(rows[i][4]),
      expiresAt: isoInfoDate(expires)
    });
  }
...
function createAnnouncement(data) {
  ...
  var message = String(data.message || "").trim().slice(0, 500);
  if (!isDriverBranch(branch)) return { success:false, error:"Choose one of the seven active branches." };
  if (!message) return { success:false, error:"Write the announcement first." };
  var created = new Date();
  var expires = new Date(created.getTime() + CONFIG.ANNOUNCEMENT_HOURS * 3600000);
  getInfoSheet("ANNOUNCEMENTS",
    ["ID","Branch","Message","Created By","Created At","Expires At"])
    .appendRow([Utilities.getUuid(), branch, message, data.createdBy || branch, created, expires]);
  return { success:true, message:"Announcement published for 24 hours" };
}

function getContacts(requestedBranch) {
  var sheet = getInfoSheet("BRANCH_CONTACTS",
    ["Branch","Name","Phone","Alternative Phone","Note","Updated By","Updated At"]);
  var rows = infoRows(sheet), byBranch = {};
  for (var i = 0; i < rows.length; i++) {
    var branch = String(rows[i][0] || "").toUpperCase().trim();
    if (!isDriverBranch(branch)) continue;
    byBranch[branch] = {
      branch: branch, name: String(rows[i][1] || ""), phone: String(rows[i][2] || ""),
      altPhone: String(rows[i][3] || ""), note: String(rows[i][4] || ""),
      updatedBy: String(rows[i][5] || ""), updatedAt: isoInfoDate(rows[i][6])
    };
  }
  var branchList = isDriverBranch(requestedBranch)
    ? [String(requestedBranch).toUpperCase().trim()]
    : DRIVER_BRANCHES;
  return branchList.map(function(branch) {
    return byBranch[branch] || { branch:branch, name:"", phone:"", altPhone:"", note:"" };
  });
}
...
function saveContact(data) {
  ...
  var phone = String(data.phone || "").trim().slice(0, 30);
  var altPhone = String(data.altPhone || "").trim().slice(0, 30);
  var note = String(data.note || "").trim().slice(0, 240);
  if (!isDriverBranch(branch)) return { success:false, error:"Choose one of the seven active branches." };
  if (!name || !phone) return { success:false, error:"Name and phone are required." };

  var sheet = getInfoSheet("BRANCH_CONTACTS",
    ["Branch","Name","Phone","Alternative Phone","Note","Updated By","Updated At"]);
  var rows = infoRows(sheet), rowNumber = -1;
  for (var i = 0; i < rows.length; i++) {
    if (String(rows[i][0] || "").toUpperCase().trim() === branch) rowNumber = i + 2;
  }
  var record = [branch, name, phone, altPhone, note, data.updatedBy || branch, new Date()];
...
function handleDriverCheckIn(data) {
  ...
  try {
    ...
    if (!isDriverBranch(branch)) return { success:false, error:"Select one of the seven active branches" };
...
function handleDriverCheckOut(data) {
  ...
  try {
    ...
    var allowedDestinations = DRIVER_DESTINATIONS[currentBranch] || [];
    if (allowedDestinations.indexOf(nextBranch) === -1) {
      return { success:false, error:"Choose one of the seven active branches from the list" };
    }
...
function handleDriverCorrectBranch(data) {
  var lock = LockService.getScriptLock();
  lock.waitLock(15000);
  try {
    var vanNo = String(data.vanNo || "").trim();
    var fromBranch = String(data.fromBranch || "").toUpperCase().trim();
    var branch = String(data.branch || "").toUpperCase().trim();
    if (!vanNo || !isDriverBranch(fromBranch) || !isDriverBranch(branch) || fromBranch === branch) {
      return { success:false, error:"Choose a different active branch" };
    }
    var state = getDriverState(vanNo);
    if (!state.started || state.status !== "AT_STATION") {
      return { success:false, error:"Only the current checked-in branch can be corrected" };
    }
    if (String(state.branch || "").toUpperCase().trim() !== fromBranch) {
      return { success:false, error:"The van state changed. Refresh before correcting it" };
    }
    var sheet = getMovementSheet();
    var values = sheet.getRange(state.rowIndex, 1, 1, MOVE_COL.UPDATED).getValues()[0];
    ...
    return {
      success:true,
      message:"Current branch corrected to " + branch,
...
function doGet(e) {
  ...
  if (action === "getAnnouncements") {
    return jsonResponse({ success:true, data:getAnnouncements() });
  }
  if (action === "getContacts") {
    return jsonResponse({ success:true, data:getContacts(e.parameter.branch) });
  }
  ...
  if (action === "getDestinations") {
    ...
    var branch = isDriverBranch(requestedBranch)
      ? requestedBranch
      : (DESTINATIONS[requestedBranch] ? requestedBranch : resolveMainBranch(requestedBranch));
    return jsonResponse({
      success:true,
      destinations: isDriverBranch(branch) ? DRIVER_DESTINATIONS[branch] : (DESTINATIONS[branch] || []),
      mainBranches: DRIVER_BRANCHES
    });
  }
...
function doPost(e) {
  ...
  switch (body.action) {
    ...
    case 'createAnnouncement': return jsonResponse(createAnnouncement(body));
    case 'saveContact':      return jsonResponse(saveContact(body));
    ...
    case 'driverCorrectBranch': return jsonResponse(handleDriverCorrectBranch(body));
